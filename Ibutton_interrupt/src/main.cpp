...
void checkSerial(char val){
  ...
  if (val > '0') {
    ...
    switch(val){
        case '1' ... '8' :
               relayCal[val-1]= current;
               Serial.printf(" Relay port %d  I = %.3F mA\n",val, relayCal[val-49]);
...
void Check_calibration(){
   NVM_values();      // load variables stored in NVM
   ...
   Serial.println("   q: Run Ibutton test !!\n");
  
    char val = 0;
    // Loop until 9 is pressed
    while (val != 'q') {
        val = readSerialChar(); // Returns 0 if nothing is pressed
